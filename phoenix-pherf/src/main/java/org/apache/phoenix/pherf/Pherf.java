...
public class Pherf {
  ...
  static {
    ...
    options.addOption("export", "export", false,
      "Exports query results to CSV files in " + PherfConstants.EXPORT_DIR + " directory");
  ...
  public Pherf(String[] args, Properties connProperties) throws Exception {
    ...
    if (connProperties != null) this.properties.putAll(connProperties);
  }
  ...
  public Pherf(String[] args) throws Exception {
    ...
    generateStatistics =
      command.hasOption("stats") ? GeneratePhoenixStats.YES : GeneratePhoenixStats.NO;
    ...
    compareType =
      command.hasOption("useAverageCompareType") ? CompareType.AVERAGE : CompareType.MINIMUM;
    ...
    if (
      (command.hasOption("h") || (args == null || args.length == 0))
  ...
  private String getLogPerNRow(CommandLine command) {
    try {
      String logPerNRows = (command.hasOption("log_per_nrows"))
        ...
        : globalProperties.getProperty(PherfConstants.LOG_PER_NROWS_NAME,
  ...
  public void run() throws Exception {
    ...
    try {
      ...
      if (applySchema) {
        ...
        SchemaReader reader =
          (schemaFile == null) ? new SchemaReader(".*.sql") : new SchemaReader(schemaFile);
        reader.applySchema();
      }
      ...
      if (monitor) {
        monitorManager = new MonitorManager(
          Integer.parseInt(globalProperties.getProperty("pherf.default.monitorFrequency")));
          ...
          if (multiTenantWorkload) {
            for (DataModel model : parser.getDataModels()) {
              for (Scenario scenario : model.getScenarios()) {
                ...
                Workload workload =
                  new MultiTenantWorkload(phoenixUtil, model, scenario, properties);
                newWorkloads.add(workload);
              }
            }
          } else {
            newWorkloads.add(new WriteWorkload(parser, properties, generateStatistics));
          }
      ...
      if (executeQuerySets) {
        ...
        workloadExecutor.add(new QueryExecutor(parser, phoenixUtil, workloadExecutor,
          parser.getDataModels(), queryHint, isFunctional, writeRuntimeResults));
