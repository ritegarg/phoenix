...
public class PherfMainIT extends ResultBaseTestIT {
  ...
  public void testPherfMain() throws Exception {
    String[] args =
      { "-q", "-l", "-z", MultiTenantTestUtils.getZookeeperFromUrl(url), "-schemaFile",
        ".*create_prod_test_unsalted.sql", "-scenarioFile", ".*prod_test_unsalted_scenario.xml" };
  ...
  public void testQueryTimeout() throws Exception {
    ...
    String[] args = { "-q", "-l", "-z", MultiTenantTestUtils.getZookeeperFromUrl(url), "-drop",
      "all", "-schemaFile", ".*timeout_test_schema.sql", "-scenarioFile",
      ".*scenario_with_query_timeouts.xml" };
  ...
  public void testRowCountMismatchException() throws Exception {
    String[] args = { "-q", "-l", "-drop", "all", "-schemaFile",
      ".*row_count_mismatch_datamodel.sql", "-scenarioFile", ".*row_count_mismatch_scenario.xml", };
    ...
    try {
      ...
      Assert.assertTrue("Expected: RowCountMismatchException, Actual: " + e,
        e instanceof RowCountMismatchException);
    }

    String[] signoff_args =
      { "-q", "-l", "-drop", "all", "-schemaFile", ".*row_count_mismatch_datamodel.sql",
        "-scenarioFile", ".*row_count_mismatch_scenario.xml", "-isSignOffTest" };
