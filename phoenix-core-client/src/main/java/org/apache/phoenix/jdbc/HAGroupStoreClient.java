...
/**
 * Write-through cache for HAGroupStore. Uses {@link PathChildrenCache} from
...
public class HAGroupStoreClient implements Closeable {
  ...
  private final ConcurrentHashMap<ClusterRoleRecord.ClusterRole,
    ConcurrentHashMap<String, ClusterRoleRecord>> clusterRoleToCRRMap = new ConcurrentHashMap<>();
  ...
  HAGroupStoreClient(final Configuration conf,
    ...
    try {
      ...
      pathChildrenCache =
        new PathChildrenCache(phoenixHaAdmin.getCurator(), ZKPaths.PATH_SEPARATOR, true);
      final CountDownLatch latch = new CountDownLatch(1);
      if (pathChildrenCacheListener != null) {
        pathChildrenCache.getListenable().addListener(pathChildrenCacheListener);
      } else {
        pathChildrenCache.getListenable().addListener((client, event) -> {
          LOGGER.info("HAGroupStoreClient PathChildrenCache Received event for type {}",
            event.getType());
          ...
          switch (event.getType()) {
            ...
            case CHILD_UPDATED:
              ...
              break;
            case CHILD_REMOVED:
              // In case of CHILD_REMOVED, we get the old version of data that was just deleted in
              // event.
              if (
                eventCRR != null && eventCRR.getHaGroupName() != null
                  && !eventCRR.getHaGroupName().isEmpty()
                  && eventCRR.getRole(phoenixHaAdmin.getZkUrl()) != null
              ) {
                LOGGER.info(
                  "Received CHILD_REMOVED event, Removing CRR {} from existing CRR Map {}",
                  eventCRR, clusterRoleToCRRMap);
      ...
      isHealthy = false;
      LOGGER.error(
        "Unexpected error occurred while initializing HAGroupStoreClient, marking cache as unhealthy",
        e);
    }
  }
  ...
  private void updateClusterRoleRecordMap(final ClusterRoleRecord crr) {
    if (
      crr != null && crr.getHaGroupName() != null && crr.getRole(phoenixHaAdmin.getZkUrl()) != null
    ) {
      ...
      for (ClusterRoleRecord.ClusterRole mapRole : clusterRoleToCRRMap.keySet()) {
        if (mapRole != role) {
          ...
          if (roleWiseMap.containsKey(crr.getHaGroupName())) {
            LOGGER.info("Removing any pre-existing mapping with role {} for HAGroupName {}",
              mapRole, crr.getHaGroupName());
  ...
  private void buildClusterRoleToCRRMap() {
    List<ClusterRoleRecord> clusterRoleRecords = pathChildrenCache.getCurrentData().stream()
      .map(this::extractCRROrNull).filter(Objects::nonNull).collect(Collectors.toList());
  ...
  public List<ClusterRoleRecord> getCRRsByClusterRole(ClusterRoleRecord.ClusterRole clusterRole)
    ...
    return ImmutableList
      .copyOf(clusterRoleToCRRMap.getOrDefault(clusterRole, new ConcurrentHashMap<>()).values());
  }
}
