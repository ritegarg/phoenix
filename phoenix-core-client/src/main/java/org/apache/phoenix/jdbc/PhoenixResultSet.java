 ...
 * Array - 1D None of the update or delete methods are supported. The ResultSet only supports the
 * following options: - ResultSet.FETCH_FORWARD - ResultSet.CONCUR_READ_ONLY -
...
public class PhoenixResultSet implements PhoenixMonitoredResultSet, SQLCloseable {
  ...
  public PhoenixResultSet(ResultIterator resultIterator, RowProjector rowProjector,
    ...
    this.queryLogger =
      context.getQueryLogger() != null ? context.getQueryLogger() : QueryLogger.NO_OP_INSTANCE;
    this.wildcardIncludesDynamicCols =
      this.context.getConnection().getQueryServices().getConfiguration()
  ...
  public void close() throws SQLException {
    ...
    try {
      ...
      if (
        context.getCurrentTable() != null && context.getCurrentTable().getTable() != null
          && !Strings
            .isNullOrEmpty(context.getCurrentTable().getTable().getPhysicalName().getString())
      ) {
        boolean isPointLookup = context.getScanRanges().isPointLookup();
        String tableName = context.getCurrentTable().getTable().getPhysicalName().toString();
  ...
  private void checkOpen() throws SQLException {
    if (isClosed) {
      throw new SQLExceptionInfo.Builder(SQLExceptionCode.RESULTSET_CLOSED).build()
        .buildException();
    }
  }
  ...
  private void checkCursorState() throws SQLException {
    ...
    if (currentRow == BEFORE_FIRST) {
      throw new SQLExceptionInfo.Builder(SQLExceptionCode.CURSOR_BEFORE_FIRST_ROW).build()
        .buildException();
    } else if (currentRow == null) {
      throw new SQLExceptionInfo.Builder(SQLExceptionCode.CURSOR_PAST_LAST_ROW).build()
        .buildException();
    }
  }
  ...
  public byte[] getBytes(int columnIndex) throws SQLException {
    ...
    byte[] value = (byte[]) projector.getValue(currentRow,
      dataType == PVarbinaryEncoded.INSTANCE ? PVarbinaryEncoded.INSTANCE : PVarbinary.INSTANCE,
      ptr);
    wasNull = (value == null);
    return value;
  }
  ...
  public float getFloat(int columnIndex) throws SQLException {
    checkCursorState();
    Float value = (Float) getRowProjector().getColumnProjector(columnIndex - 1).getValue(currentRow,
      PFloat.INSTANCE, ptr);
    wasNull = (value == null);
    if (wasNull) {
      return 0;
  ...
  public Object getObject(int columnIndex) throws SQLException {
    ...
    if (isApplyTimeZoneDisplacement) {
      ...
      if (type == PDate.INSTANCE || type == PUnsignedDate.INSTANCE) {
        value =
          DateUtil.applyOutputDisplacement((java.sql.Date) value, localCalendar.getTimeZone());
      } else if (type == PTime.INSTANCE || type == PUnsignedTime.INSTANCE) {
        value =
          DateUtil.applyOutputDisplacement((java.sql.Time) value, localCalendar.getTimeZone());
      } else if (type == PTimestamp.INSTANCE || type == PUnsignedTimestamp.INSTANCE) {
        value =
          DateUtil.applyOutputDisplacement((java.sql.Timestamp) value, localCalendar.getTimeZone());
      }
    }
    return value;
  }
  ...
  public short getShort(int columnIndex) throws SQLException {
    checkCursorState();
    Short value = (Short) getRowProjector().getColumnProjector(columnIndex - 1).getValue(currentRow,
      PSmallint.INSTANCE, ptr);
    wasNull = (value == null);
    if (wasNull) {
      return 0;
    }
  ...
  public URL getURL(int columnIndex) throws SQLException {
    ...
    try {
      ...
      throw new SQLExceptionInfo.Builder(SQLExceptionCode.MALFORMED_URL).setRootCause(e).build()
        .buildException();
    }
  }
  ...
  private void updateTableLevelReadMetrics(String tableName, boolean isPointLookup) {
    ...
    if (readMetrics.get(tableName) != null) {
      ...
      TableMetricsManager.updateHistogramMetricsForQueryScanBytes(scanBytes, tableName,
        isPointLookup);
      ...
      TableMetricsManager.updateMetricsMethod(tableName,
        this.exception == null
          ...
          : MetricType.SELECT_AGGREGATE_FAILURE_SQL_COUNTER,
        1);
    }
  }
   ...
   * @return the row projector including dynamic column projectors in case we are including dynamic
   *         columns, otherwise the regular row projector containing static column projectors
   */
   ...
   * @return Pair whose first part is the list of static column PColumns and the second part is the
   *         starting position for dynamic columns
   */
  ...
  private void processDynamicColumnsIfRequired() {
    if (
      !this.wildcardIncludesDynamicCols || this.currentRow == null
   ...
   * the deserialized list of dynamic column PColumns for the current row
  ...
  private List<PColumn> getDynColsListAndSeparateFromActualData() {
    ...
    final byte[] anchor =
      Arrays.copyOf(DYN_COLS_METADATA_CELL_QUALIFIER, DYN_COLS_METADATA_CELL_QUALIFIER.length);
    ...
    byte[] actualCellDataBytes =
      Arrays.copyOfRange(valueArray, 0, pos - DYN_COLS_METADATA_CELL_QUALIFIER.length);
    ...
    this.currentRow = new TupleProjector.ProjectedValueTuple(key.get(), key.getOffset(),
      key.getLength(), base.getTimestamp(), actualCellData.get(), actualCellData.getOffset(),
      actualCellData.getLength(), 0);

    byte[] dynColsListBytes = Arrays.copyOfRange(valueArray, pos, valueArray.length);
    List<PColumn> dynCols = new ArrayList<>();
    try {
      List<PTableProtos.PColumn> dynColsProtos = DynamicColumnMetaDataProtos.DynamicColumnMetaData
        .parseFrom(dynColsListBytes).getDynamicColumnsList();
  ...
  private RowProjector mergeRowProjectorWithDynColProjectors(List<PColumn> dynCols,
    ...
    for (PColumn currentDynCol : dynCols) {
      ...
      Expression exp = new ProjectedColumnExpression(currentDynCol, allCols, startingPos++,
        currentDynCol.getName().getString());

      ColumnProjector dynColProj = new ExpressionProjector(currentDynCol.getName().getString(),
    ...
    return new RowProjector(allColumnProjectors, this.rowProjector.getEstimatedRowByteSize(),
      ...
      this.rowProjector.projectEverything(), this.rowProjector.projectDynColsInWildcardQueries());
  }
