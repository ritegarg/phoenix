...
public class HashCacheClient {
  ...
  /**
   * Creates a ServerCache object for cacheId. This is used for persistent cache, and there may or
   * may not be corresponding data on each region server.
   * @param cacheId  ID for the cache entry
   * @param delegate the query plan this will be used for
   * @return client-side {@link ServerCache} representing the hash cache that may or may not be
   *         present on region servers. size
   */
  ...
  /**
   * Send the results of scanning through the scanner to all region servers for regions of the table
   * that will use the cache that intersect with the minMaxKeyRange.
   ...
   */
  public ServerCache addHashCache(ScanRanges keyRanges, byte[] cacheId, ResultIterator iterator,
    long estimatedSize, List<Expression> onExpressions, boolean singleValueOnly,
    boolean usePersistentCache, PTable cacheUsingTable, Expression keyRangeRhsExpression,
    ...
    serialize(ptr, iterator, estimatedSize, onExpressions, singleValueOnly, keyRangeRhsExpression,
      keyRangeRhsValues);
    ServerCache cache = serverCache.addServerCache(keyRanges, cacheId, ptr,
      ByteUtil.EMPTY_BYTE_ARRAY, new HashCacheFactory(), cacheUsingTable, usePersistentCache, true);
    return cache;
  }
   ...
   */
  public boolean addHashCacheToServer(byte[] startkeyOfRegion, ServerCache cache, PTable pTable)
    throws Exception {
    if (cache == null) {
      return false;
    }
    return serverCache.addServerCache(startkeyOfRegion, cache, new HashCacheFactory(),
      ByteUtil.EMPTY_BYTE_ARRAY, pTable);
  }
  ...
  private void serialize(ImmutableBytesWritable ptr, ResultIterator iterator, long estimatedSize,
    ...
    List<Expression> keyRangeRhsValues) throws SQLException {
    long maxSize = serverCache.getConnection().getQueryServices().getProps().getLongBytes(
  ...
  /**
   * Evaluate the RHS key expression and wrap the result as a new Expression. Unlike other types of
   * Expression which will be evaluated and wrapped as a single LiteralExpression,
   * RowValueConstructorExpression should be handled differently. We should evaluate each child of
   * RVC and wrap them into a new RVC Expression, in order to make sure that the later coercion
   * between the LHS key expression and this RHS key expression will be successful.
   ...
   */
  public static Expression evaluateKeyExpression(Expression keyExpression, Tuple tuple,
