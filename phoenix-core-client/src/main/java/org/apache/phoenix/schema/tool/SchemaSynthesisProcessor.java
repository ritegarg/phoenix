...
public class SchemaSynthesisProcessor implements SchemaProcessor {
  public static final String ENTITY_NAME_IN_BASE_AND_ALTER_DDL_DON_T_MATCH =
    "Entity name in base and alter DDL don't match";
  public static final String UNSUPPORTED_DDL_EXCEPTION =
  ...
  private String synthesize(String baseDDL, String nextDDL) throws Exception {
    ...
    if (createStatement instanceof CreateTableStatement) {
      ...
      if (alterStatement instanceof AddColumnStatement) {
        newCreateStmt = getCreateTableStatement((AddColumnStatement) alterStatement, createStmt);
      } else if (alterStatement instanceof DropColumnStatement) {
        newCreateStmt = getCreateTableStatement((DropColumnStatement) alterStatement, createStmt);
  ...
  private CreateIndexStatement getCreateIndexStatement(BindableStatement alterStatement,
    CreateIndexStatement createStatement) throws Exception {
  ...
  private CreateTableStatement getCreateTableStatement(AddColumnStatement alterStatement,
    ...
    if (addStmt.getColumnDefs() == null) {
      ListMultimap<String, Pair<String, Object>> finalProps =
        getEffectiveProperties(addStmt, createStmt.getProps());
      ...
      for (Pair<ColumnName, SortOrder> entry : oldPKConstraint.getColumnNames()) {
        ColumnDefInPkConstraint cd = new ColumnDefInPkConstraint(entry.getFirst(),
          entry.getSecond(), oldPKConstraint.isColumnRowTimestamp(entry.getFirst()));
        pkList.add(cd);
      }
      for (ColumnDef cd : addStmt.getColumnDefs()) {
        if (cd.isPK()) {
          ColumnDefInPkConstraint cdpk = new ColumnDefInPkConstraint(cd.getColumnDefName(),
            cd.getSortOrder(), cd.isRowTimestamp());
          pkList.add(cdpk);
        }
      }
      PrimaryKeyConstraint pkConstraint =
        new PrimaryKeyConstraint(oldPKConstraint.getName(), pkList);
