 ...
 * tabe Upto #DEFAULT_MAX_META_DATA_VERSIONS versions are kept. The time stamp of the metadata must
 * always be increasing. The timestamp of the key values in the data row corresponds to the schema
 * that it's using. TODO: dynamically prune number of schema version kept based on whether or not
 * the data table still uses it (based on the min time stamp of the data table).
 * @since 0.1
 */
public abstract class MetaDataProtocol extends MetaDataService {
  ...
  public static class SharedTableState {
    ...
    public SharedTableState(
      ...
      this.tenantId = sharedTable.hasTenantId()
        ? PNameFactory.newName(sharedTable.getTenantId().toByteArray())
        : null;
      ...
      this.columns = Lists.transform(sharedTable.getColumnsList(),
        new Function<org.apache.phoenix.coprocessor.generated.PTableProtos.PColumn, PColumn>() {
          @Override
          public PColumn
            apply(org.apache.phoenix.coprocessor.generated.PTableProtos.PColumn column) {
            return PColumnImpl.createFromProto(column);
          }
        });
      this.physicalNames =
        Lists.transform(sharedTable.getPhysicalNamesList(), new Function<ByteString, PName>() {
          @Override
  ...
  public static class MetaDataMutationResult {
    ...
    public MetaDataMutationResult(MutationCode returnCode, long currentTime,
      List<PFunction> functions, boolean wasUpdated) {
    ...
    public MetaDataMutationResult(MutationCode returnCode, int currentTime, PTable table,
      long viewIndexId, PDataType viewIndexIdType) {
      this(returnCode, currentTime, table, Collections.<byte[]> emptyList());
    ...
    public static MetaDataMutationResult constructFromProto(MetaDataResponse proto) {
      ...
      if (proto.getSharedTablesToDeleteCount() > 0) {
        ...
        for (org.apache.phoenix.coprocessor.generated.MetaDataProtos.SharedTableState sharedTable : proto
    ...
    public static MetaDataResponse toProto(MetaDataMutationResult result) {
      ...
      if (result != null) {
        builder
          .setReturnCode(MetaDataProtos.MutationCode.values()[result.getMutationCode().ordinal()]);
        ...
        if (result.getSharedTablesToDelete() != null) {
          for (SharedTableState sharedTableState : result.sharedTablesToDelete) {
            ...
            for (PName physicalName : sharedTableState.getPhysicalNames()) {
              sharedTableStateBuilder
                .addPhysicalNames(UnsafeByteOperations.unsafeWrap(physicalName.getBytes()));
            }
            if (sharedTableState.getTenantId() != null) {
              sharedTableStateBuilder.setTenantId(
                UnsafeByteOperations.unsafeWrap(sharedTableState.getTenantId().getBytes()));
            }
            sharedTableStateBuilder.setSchemaName(
              UnsafeByteOperations.unsafeWrap(sharedTableState.getSchemaName().getBytes()));
            sharedTableStateBuilder.setTableName(
              UnsafeByteOperations.unsafeWrap(sharedTableState.getTableName().getBytes()));
            sharedTableStateBuilder.setViewIndexId(sharedTableState.getViewIndexId());
            sharedTableStateBuilder
              .setViewIndexIdType(sharedTableState.viewIndexIdType.getSqlType());
  ...
  public static String getVersion(long serverTimestamp) {
    /*
     * It is possible that when clients are trying to run upgrades concurrently, we could be at an
     * intermediate server timestamp. Using floorKey provides us a range based lookup where the
     * timestamp range for a release is [timeStampForRelease, timestampForNextRelease).
     */
