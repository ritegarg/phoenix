...
public class IndexUtil {
  ...
  public static final String PHOENIX_INDEX_BUILDER_CLASSNAME =
    "org.apache.phoenix.index.PhoenixIndexBuilder";

  /**
   * Configuration key for the {@link org.apache.phoenix.hbase.index.builder.IndexBuilder} to use
   */
  public static final String INDEX_BUILDER_CONF_KEY = "index.builder";

  private final static Cache<String,
    Boolean> indexNameGlobalIndexCheckerEnabledMap = CacheBuilder.newBuilder()
  ...
  public static PDataType getIndexColumnDataType(PColumn dataColumn) throws SQLException {
    ...
    if (type == null) {
      throw new SQLExceptionInfo.Builder(SQLExceptionCode.CANNOT_INDEX_COLUMN_ON_TYPE)
        .setColumnName(dataColumn.getName().getString())
  ...
  public static String getCaseSensitiveDataColumnFullName(String name) {
    ...
    return SchemaUtil.getCaseSensitiveColumnDisplayName(getDataColumnFamilyName(name),
  ...
  public static byte[] getIndexColumnName(byte[] dataColumnFamilyName, byte[] dataColumnName) {
    return ByteUtil.concat(
      dataColumnFamilyName == null ? ByteUtil.EMPTY_BYTE_ARRAY : dataColumnFamilyName,
      INDEX_COLUMN_NAME_SEP_BYTES, dataColumnName);
  }

  public static String getIndexColumnName(PColumn dataColumn) {
    String dataColumnFamilyName =
      SchemaUtil.isPKColumn(dataColumn) ? null : dataColumn.getFamilyName().getString();
  ...
  public static PColumn getIndexPKColumn(int position, PColumn dataColumn) {
    ...
    PName indexColumnName =
      PNameFactory.newName(getIndexColumnName(null, dataColumn.getName().getString()));
    PColumn column = new PColumnImpl(indexColumnName, null, dataColumn.getDataType(),
      dataColumn.getMaxLength(), dataColumn.getScale(), dataColumn.isNullable(), position,
      dataColumn.getSortOrder(), dataColumn.getArraySize(), null, false,
  ...
  public static String getLocalIndexColumnFamily(String dataColumnFamilyName) {
    return dataColumnFamilyName == null
      ? null
  ...
  public static PColumn getDataColumn(PTable dataTable, String indexColumnName) {
    ...
    if (column == null) {
      throw new IllegalArgumentException("Could not find column \"" + SchemaUtil
        .getColumnName(getDataColumnFamilyName(indexColumnName), getDataColumnName(indexColumnName))
        + " in " + dataTable);
    }
    return column;
  }
  ...
  /**
   * Return a list of {@code PColumn} for the associated data columns given the corresponding index
   * columns. For a tenant specific view, the connection needs to be tenant specific too.
   * @throws TableNotFoundException if table cannot be found in the connection's metdata cache
   */
  public static List<PColumn> getDataColumns(String dataTableName, List<PColumn> indexColumns,
    PhoenixConnection conn) throws SQLException {
  ...
  public static boolean isEmptyKeyValue(PTable table, ColumnReference ref) {
    ...
    return (Bytes.compareTo(emptyKeyValueCF, 0, emptyKeyValueCF.length,
      ref.getFamilyWritable().get(), ref.getFamilyWritable().getOffset(),
      ref.getFamilyWritable().getLength()) == 0
      && Bytes.compareTo(emptyKeyValueQualifier, 0, emptyKeyValueQualifier.length,
        ref.getQualifierWritable().get(), ref.getQualifierWritable().getOffset(),
        ref.getQualifierWritable().getLength()) == 0);
  }
  ...
  public static List<Mutation> generateIndexData(final PTable table, PTable index,
    final MultiRowMutationState multiRowMutationState, List<Mutation> dataMutations,
    final KeyValueBuilder kvBuilder, PhoenixConnection connection)
    ...
    try {
      ...
      for (final Mutation dataMutation : dataMutations) {
         ...
         * Deletes of rows are handled by running a re-written query against the index table, and
         * Deletes of column values should never be necessary, as you should never be updating an
         * existing row.
         */
        if (dataMutation instanceof Put) {
          ValueGetter valueGetter = new AbstractValueGetter() {
            ...
            public ImmutableBytesWritable getLatestValue(ColumnReference ref, long ts) {
              ...
              for (Cell kv : kvs) {
                if (
                  Bytes.compareTo(kv.getFamilyArray(), kv.getFamilyOffset(), kv.getFamilyLength(),
          ...
          if (maintainer.isLocalIndex()) {
            HRegionLocation tableRegionLocation = connection.getQueryServices()
              .getTableRegionLocation(table.getPhysicalName().getBytes(), dataMutation.getRow());
  ...
  /**
   * Traverse the expression tree and set the offset of every RowKeyColumnExpression to the offset
   * provided. This is used for local indexing on the server-side to skip over the region start key
   * that prefixes index rows.
  ...
  public static ColumnReference[] deserializeDataTableColumnsToJoin(Scan scan) {
    byte[] columnsBytes =
      scan.getAttribute(BaseScannerRegionObserverConstants.DATA_TABLE_COLUMNS_TO_JOIN);
    ...
    try {
      ...
      for (int i = 0; i < numColumns; i++) {
        dataColumns[i] =
          new ColumnReference(Bytes.readByteArray(input), Bytes.readByteArray(input));
      }
      return dataColumns;
    } catch (IOException e) {
  ...
  public static KeyValueSchema deserializeLocalIndexJoinSchemaFromScan(final Scan scan) {
    byte[] schemaBytes =
      scan.getAttribute(BaseScannerRegionObserverConstants.LOCAL_INDEX_JOIN_SCHEMA);
  ...
  public static TupleProjector getTupleProjector(Scan scan, ColumnReference[] dataColumns) {
    if (dataColumns != null && dataColumns.length != 0) {
      ...
      boolean storeColsInSingleCell =
        scan.getAttribute(BaseScannerRegionObserverConstants.COLUMNS_STORED_IN_SINGLE_CELL) != null;
      ...
      Expression[] colExpressions = storeColsInSingleCell
        ? new SingleCellColumnExpression[dataColumns.length]
        : new KeyValueColumnExpression[dataColumns.length];
      for (int i = 0; i < dataColumns.length; i++) {
        byte[] family = dataColumns[i].getFamily();
        byte[] qualifier = dataColumns[i].getQualifier();
        Field field = keyValueSchema.getField(i);
        Expression dataColumnExpr = storeColsInSingleCell
          ? new SingleCellColumnExpression(field, family, qualifier, encodingScheme,
   ...
   */
  public static String rewriteViewStatement(PhoenixConnection conn, PTable index, PTable table,
    ...
    TableRef indexTableRef = new TableRef(index) {
      @Override
      public String getColumnDisplayName(ColumnRef ref, boolean schemaNameCaseSensitive,
        boolean colNameCaseSensitive) {
    ...
    return QueryUtil.getViewStatement(index.getSchemaName().getString(),
      index.getTableName().getString(), buf.toString());
  }

  public static void addTupleAsOneCell(List<Cell> result, Tuple tuple,
    TupleProjector tupleProjector, ImmutableBytesWritable ptr) {
    // This will create a byte[] that captures all of the values from the data table
    byte[] value = tupleProjector.getSchema().toBytes(tuple, tupleProjector.getExpressions(),
      tupleProjector.getValueBitSet(), ptr);
    Cell firstCell = result.get(0);
    Cell keyValue = PhoenixKeyValueUtil.newKeyValue(firstCell.getRowArray(), // FIXME: This does
                                                                             // DEEP_COPY of cell,
                                                                             // do we need that?
  ...
  public static byte[][] getViewConstants(PTable dataTable) {
    if (dataTable.getType() != PTableType.VIEW && dataTable.getType() != PTableType.PROJECTED)
      return null;
    int dataPosOffset =
      (dataTable.getBucketNum() != null ? 1 : 0) + (dataTable.isMultiTenant() ? 1 : 0);
  ...
  public static MetaDataMutationResult updateIndexState(byte[] indexTableKey, long minTimeStamp,
    ...
    put.addColumn(PhoenixDatabaseMetaData.TABLE_FAMILY_BYTES,
      PhoenixDatabaseMetaData.INDEX_STATE_BYTES, newState.getSerializedBytes());
    put.addColumn(PhoenixDatabaseMetaData.TABLE_FAMILY_BYTES,
    ...
    final Map<byte[], MetaDataResponse> results =
      metaTable.coprocessorService(MetaDataService.class, indexTableKey, indexTableKey,
  ...
  public static PTable getPDataTable(PhoenixConnection conn, TableDescriptor tableDesc)
    ...
    if (dataTableName == null) {
      if (physicalTableName.contains(QueryConstants.NAMESPACE_SEPARATOR)) {
        try {
          pDataTable = conn.getTable(physicalTableName.replace(QueryConstants.NAMESPACE_SEPARATOR,
  ...
  public static void updateIndexState(PhoenixConnection conn, String indexTableName,
    ...
    updateIndexState(conn, indexTableName, newState, indexDisableTimestamp,
      HConstants.LATEST_TIMESTAMP);
  }

  public static void updateIndexState(PhoenixConnection conn, String indexTableName,
    PIndexState newState, Long indexDisableTimestamp, Long expectedMaxTimestamp)
    throws SQLException {
    byte[] indexTableKey = SchemaUtil.getTableKeyFromFullName(indexTableName);
    ...
    Put put = new Put(indexTableKey);
    put.addColumn(PhoenixDatabaseMetaData.TABLE_FAMILY_BYTES,
      PhoenixDatabaseMetaData.INDEX_STATE_BYTES, expectedMaxTimestamp,
      newState.getSerializedBytes());
    if (indexDisableTimestamp != null) {
      put.addColumn(PhoenixDatabaseMetaData.TABLE_FAMILY_BYTES,
        PhoenixDatabaseMetaData.INDEX_DISABLE_TIMESTAMP_BYTES, expectedMaxTimestamp,
    ...
    if (code == MutationCode.UNALLOWED_TABLE_MUTATION) {
      throw new SQLExceptionInfo.Builder(SQLExceptionCode.INVALID_INDEX_STATE_TRANSITION)
        .setMessage("indexState=" + newState).setSchemaName(schemaName).setTableName(indexName)
  ...
  public static Result incrementCounterForIndex(PhoenixConnection conn, String failedIndexTable,
    ...
    try (Table table = conn.getQueryServices()
  ...
  public static long getIndexPendingDisableCount(PhoenixConnection conn, String failedIndexTable)
    throws IOException {
    byte[] indexTableKey = SchemaUtil.getTableKeyFromFullName(failedIndexTable);
    ...
    try (Table table = conn.getQueryServices()
      ...
      return Bytes.toLong(
        result.getValue(TABLE_FAMILY_BYTES, PhoenixDatabaseMetaData.PENDING_DISABLE_COUNT_BYTES));
    } catch (SQLException e) {
      throw new IOException(e);
    }
  }

  public static long getIndexPendingDisableCountLastUpdatedTimestamp(PhoenixConnection conn,
    String failedIndexTable) throws IOException {
    byte[] indexTableKey = SchemaUtil.getTableKeyFromFullName(failedIndexTable);
    Get get = new Get(indexTableKey);
    get.addColumn(TABLE_FAMILY_BYTES, PhoenixDatabaseMetaData.PENDING_DISABLE_COUNT_BYTES);
    byte[] systemCatalog =
      SchemaUtil.getPhysicalTableName(PhoenixDatabaseMetaData.SYSTEM_CATALOG_NAME,
        conn.getQueryServices().getProps()).getName();
  ...
  public static int getIndexPriority(Configuration conf) {
    return conf.getInt(QueryServices.INDEX_PRIOIRTY_ATTRIB,
      QueryServicesOptions.DEFAULT_INDEX_PRIORITY);
  }

  public static int getMetadataPriority(Configuration conf) {
    return conf.getInt(QueryServices.METADATA_PRIOIRTY_ATTRIB,
      QueryServicesOptions.DEFAULT_METADATA_PRIORITY);
  }

  public static int getServerSidePriority(Configuration conf) {
    return conf.getInt(QueryServices.SERVER_SIDE_PRIOIRTY_ATTRIB,
      QueryServicesOptions.DEFAULT_SERVER_SIDE_PRIORITY);
  }
   ...
   * @param descBuilder           {@link TableDescriptor} for the table on which indexing should be
   *                              enabled
   ...
   *                              {@link org.apache.phoenix.hbase.index.builder.IndexBuilder} on the
   *                              server-side
   * @param priority              TODO
  ...
  public static void enableIndexing(TableDescriptorBuilder descBuilder,
    String indexBuilderClassName, Map<String, String> properties, int priority,
    String coprocessorClassName) throws IOException {
    if (properties == null) {
      properties = new HashMap<String, String>();
    }
    properties.put(IndexUtil.INDEX_BUILDER_CONF_KEY, indexBuilderClassName);
    descBuilder.setCoprocessor(CoprocessorDescriptorBuilder.newBuilder(coprocessorClassName)
  ...
  public static class IndexStatusUpdater {
    ...
    private void updateVerified(Cell cell) {
      if (
        CellUtil.compareFamilies(cell, emptyKeyValueCF, 0, emptyKeyValueCFLength) == 0 && CellUtil
          .compareQualifiers(cell, emptyKeyValueQualifier, 0, emptyKeyValueQualifierLength) == 0
      ) {
