...
/**
 * Client side Aggregator which will aggregate data and find distinct values with number of
...
public abstract class DistinctValueWithCountClientAggregator extends BaseAggregator {
  ...
  public void aggregate(Tuple tuple, ImmutableBytesWritable ptr) {
    if (tuple instanceof SingleKeyValueTuple) {
      // Case when scanners do look ahead and re-aggregate result row.The result is already
      // available in the ptr
      ...
      try {
        if (
          Bytes.equals(ptr.get(), ptr.getOffset(), 1,
          ...
          int uncompressedLength =
            Snappy.uncompressedLength(ptr.get(), ptr.getOffset() + 1, ptr.getLength() - 1);
  ...
  protected Map<Object, Integer> getSortedValueVsCount(final boolean ascending,
