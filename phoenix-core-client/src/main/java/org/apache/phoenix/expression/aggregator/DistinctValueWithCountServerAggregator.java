...
/**
 * Server side Aggregator which will aggregate data and find distinct values with number of
...
public class DistinctValueWithCountServerAggregator extends BaseAggregator {
  ...
  protected Map<ImmutableBytesPtr, Integer> valueVsCount =
    new HashMap<ImmutableBytesPtr, Integer>();
  ...
  public DistinctValueWithCountServerAggregator(Configuration conf,
    DistinctValueWithCountClientAggregator clientAgg) {
    this(conf);
  ...
  public void aggregate(Tuple tuple, ImmutableBytesWritable ptr) {
    ImmutableBytesPtr key =
      ptr.get().length > FIXED_COPY_THRESHOLD && ptr.get().length > ptr.getLength() * COPY_THRESHOLD
  ...
  public boolean evaluate(Tuple tuple, ImmutableBytesWritable ptr) {
    ...
    if (serializationSize > compressThreshold) {
      // The size for the map serialization is above the threshold. We will do the Snappy
      // compression here.
      byte[] compressed =
        new byte[COMPRESS_MARKER.length + Snappy.maxCompressedLength(buffer.length)];
      System.arraycopy(COMPRESS_MARKER, 0, compressed, 0, COMPRESS_MARKER.length);
      try {
        int compressedLen =
          Snappy.compress(buffer, 1, buffer.length - 1, compressed, COMPRESS_MARKER.length);
