...
public class TupleProjector {
   ...
   * @param dynColCellQualifiers Populated set of <column family, column qualifier> pairs for the
   *                             cells in the list, which correspond to dynamic columns
   * @throws InvalidProtocolBufferException Thrown if there is an error parsing byte[] to protobuf
   */
  private static void populateDynColsFromResult(List<Cell> result, List<PColumn> dynCols,
    Set<Pair<ByteBuffer, ByteBuffer>> dynColCellQualifiers) throws InvalidProtocolBufferException {
    for (Cell c : result) {
      ...
      if (index != -1) {
        ...
        dynCols
          .add(PColumnImpl.createFromProto(PTableProtos.PColumn.parseFrom(dynColMetaDataProto)));
        ...
        byte[] dynColQual =
          Arrays.copyOfRange(qual, index + DYN_COLS_METADATA_CELL_QUALIFIER.length, qual.length);
        dynColCellQualifiers.add(new Pair<>(ByteBuffer.wrap(fam), ByteBuffer.wrap(dynColQual)));
      }
    }
  }
  ...
  public static class ProjectedValueTuple extends BaseTuple {
    ...
    public ProjectedValueTuple(Tuple keyBase, long timestamp, byte[] projectedValue,
      int valueOffset, int valueLength, int bitSetLen) {
    ...
    public ProjectedValueTuple(byte[] keyBuffer, int keyOffset, int keyLength, long timestamp,
    ...
    public Cell mergeWithDynColsListBytesAndGetValue(int index, byte[] dynColsList) {
      ...
      byte[] concatBytes =
        ByteUtil.concat(projectedValue.get(), DYN_COLS_METADATA_CELL_QUALIFIER, dynColsList);
      ...
      keyValue = PhoenixKeyValueUtil.newKeyValue(keyPtr.get(), keyPtr.getOffset(),
        ...
        projectedValueWithDynColsListBytes.get(), projectedValueWithDynColsListBytes.getOffset(),
    ...
    public Cell getValue(byte[] family, byte[] qualifier) {
      if (keyValue == null) {
        keyValue = PhoenixKeyValueUtil.newKeyValue(keyPtr.get(), keyPtr.getOffset(),
          keyPtr.getLength(), VALUE_COLUMN_FAMILY, VALUE_COLUMN_QUALIFIER, timestamp,
          projectedValue.get(), projectedValue.getOffset(), projectedValue.getLength());
      }
      return keyValue;
    }
  ...
  public static class OldProjectedValueTuple extends ProjectedValueTuple {
    ...
    public OldProjectedValueTuple(byte[] keyBuffer, int keyOffset, int keyLength, long timestamp,
      byte[] projectedValue, int valueOffset, int valueLength, int bitSetLen) {
      super(keyBuffer, keyOffset, keyLength, timestamp, projectedValue, valueOffset, valueLength,
        bitSetLen);
    }

    public OldProjectedValueTuple(Tuple keyBase, long timestamp, byte[] projectedValue,
      int valueOffset, int valueLength, int bitSetLen) {
    ...
    public Cell getValue(byte[] family, byte[] qualifier) {
      if (keyValue == null) {
        keyValue = PhoenixKeyValueUtil.newKeyValue(keyPtr.get(), keyPtr.getOffset(),
          keyPtr.getLength(), VALUE_COLUMN_FAMILY, OLD_VALUE_COLUMN_QUALIFIER, timestamp,
          projectedValue.get(), projectedValue.getOffset(), projectedValue.getLength());
      }
      return keyValue;
    }
  ...
  public ProjectedValueTuple projectResults(Tuple tuple, boolean useNewValueQualifier) {
    ...
    if (useNewValueQualifier) {
      ...
      return new OldProjectedValueTuple(base.getRowArray(), base.getRowOffset(),
        base.getRowLength(), maxTS, bytesValue, 0, bytesValue.length,
  ...
  public static ProjectedValueTuple mergeProjectedValue(ProjectedValueTuple dest,
    ...
    byte[] merged =
      new byte[destValue.getLength() - origDestBitSetLen + srcValueLen + destBitSetLen];
    int o = Bytes.putBytes(merged, 0, destValue.get(), destValue.getOffset(),
    ...
    return useNewValueColumnQualifier
      ? new ProjectedValueTuple(dest, dest.getTimestamp(), merged, 0, merged.length, destBitSetLen)
      : new OldProjectedValueTuple(dest, dest.getTimestamp(), merged, 0, merged.length,
        destBitSetLen);
  }
