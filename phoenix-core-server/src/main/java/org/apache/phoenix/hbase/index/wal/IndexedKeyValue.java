...
public class IndexedKeyValue extends KeyValue {
  ...
  private static Cell adaptFirstCellFromMutation(Mutation m) {
    if (
      m != null && m.getFamilyCellMap() != null && m.getFamilyCellMap().firstEntry() != null
      ...
      return builder.setFamily(WALEdit.METAFAMILY).setQualifier(mutationCell.getQualifierArray())
        .setRow(m.getRow()).setTimestamp(mutationCell.getTimestamp())
        .setValue(mutationCell.getValueArray()).setType(Cell.Type.Put).build();
    } else {
      throw new IllegalArgumentException(
        "Tried to create an IndexedKeyValue with a " + "Mutation with no Cells!");
    }
   ...
   * consistent reading/writing of {@link IndexedKeyValue}s. to write data to. Does not close or
   * flush the passed object. if there is a problem writing the underlying data
   */
  ...
  protected MutationProto toMutationProto(Mutation mutation) throws IOException {
    ...
    if (mutation instanceof Put) {
      m =
        org.apache.hadoop.hbase.shaded.protobuf.ProtobufUtil.toMutation(MutationType.PUT, mutation);
