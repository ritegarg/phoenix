...
public class PhoenixAccessController extends BaseMetaDataEndpointObserver {
    ...
    if (oldAccessControllers == null) {
      ...
      for (RegionCoprocessor cp : cpHost.findCoprocessors(RegionCoprocessor.class)) {
        if (cp instanceof AccessControlService.Interface && cp instanceof MasterObserver) {
          ...
          if (
            cp.getClass().getName()
              .equals(org.apache.hadoop.hbase.security.access.AccessController.class.getName())
          ) {
  ...
  public ObserverContext<MasterCoprocessorEnvironment> getMasterObsevrverContext()
    throws IOException {
    return new ObserverContextImpl<MasterCoprocessorEnvironment>(getActiveUser());
  }

  @Override
  public void preGetTable(ObserverContext<PhoenixMetaDataControllerEnvironment> ctx,
  ...
  public void preCreateTable(ObserverContext<PhoenixMetaDataControllerEnvironment> ctx,
    ...
    if (
      tableType == PTableType.VIEW || tableType == PTableType.INDEX || tableType == PTableType.CDC
    ) {
    ...
    if (tableType == PTableType.VIEW) {
      ...
      Action[] requiredActions = execPermissionsCheckEnabled
        ? new Action[] { Action.READ, Action.EXEC }
        : new Action[] { Action.READ };
      for (TableName index : indexes) {
        ...
        List<UserPermission> permissionForUser =
          getPermissionForUser(getUserPermissions(index), user.getShortName());
    ...
    if (tableType == PTableType.INDEX) {
      ...
      // We are doing this because existing user while querying data table should not see access
      // denied for the
      // new indexes.
      // TODO: confirm whether granting permission from coprocessor is a security leak.(currently it
      // is done if
      // automatic grant is enabled explicitly by user in configuration
      // skip check for local index
      if (
        physicalTableName != null && !parentPhysicalTableName.equals(physicalTableName)
          && !MetaDataUtil.isViewIndex(physicalTableName.getNameAsString())
      ) {
        List<Action> actions =
          new ArrayList<>(Arrays.asList(Action.READ, Action.WRITE, Action.CREATE, Action.ADMIN));
        if (execPermissionsCheckEnabled) {
          actions.add(Action.EXEC);
        }
        authorizeOrGrantAccessToUsers("Create" + tableType, parentPhysicalTableName, actions,
          physicalTableName);
      }
    }
  }

  public void handleRequireAccessOnDependentTable(String request, String userName,
    TableName dependentTable, String requestTable, Set<Action> requireAccess,
    Set<Action> accessExists) throws IOException {

    Set<Action> unionSet = new HashSet<Action>();
    unionSet.addAll(requireAccess);
    unionSet.addAll(accessExists);
    AUDITLOG
      .info(request + ": Automatically granting access to index table during creation of view:"
  ...
  private void grantPermissions(final String toUser, final byte[] table, final Action... actions)
    throws IOException {
  ...
  private void authorizeOrGrantAccessToUsers(final String request, final TableName fromTable,
    final List<Action> requiredActionsOnTable, final TableName toTable) throws IOException {
    User.runAsLoginUser(new PrivilegedExceptionAction<Void>() {
      @Override
          ...
          for (UserPermission userPermission : userPermissions) {
            ...
            if (!requireAccess.isEmpty()) {
              if (AuthUtil.isGroupPrincipal(userPermission.getUser())) {
                AUDITLOG.warn(
                  "Users of GROUP:" + userPermission.getUser() + " will not have following access "
                    + requireAccess + " to the newly created index " + toTable
                    + ", Automatic grant is not yet allowed on Groups");
                continue;
              }
              handleRequireAccessOnDependentTable(request, userPermission.getUser(), toTable,
  ...
  private List<UserPermission> getPermissionForUser(List<UserPermission> perms, String user) {
    if (perms != null) {
      // get list of permissions for the user as multiple implementation of AccessControl
      // coprocessors can give
  ...
  public void preAlterTable(ObserverContext<PhoenixMetaDataControllerEnvironment> ctx,
    String tenantId, String tableName, TableName physicalTableName,
    TableName parentPhysicalTableName, PTableType tableType) throws IOException {
    if (!accessCheckEnabled) {
      return;
    }
    for (MasterObserver observer : getAccessControllers()) {
  ...
  /**
   * Gets all the permissions for a given tableName for all the users Also, get the permissions at
   * table's namespace level and merge all of them
   */
        ...
        public List<UserPermission> run() throws Exception {
          ...
          try {
            ...
            for (MasterObserver service : getAccessControllers()) {
              // Use AccessControlClient API's if the accessController is an instance of
              // org.apache.hadoop.hbase.security.access.AccessController
              if (
                service.getClass().getName()
                  .equals(org.apache.hadoop.hbase.security.access.AccessController.class.getName())
              ) {
                userPermissions.addAll(AccessControlClient.getUserPermissions(serverConnection,
  ...
  private void getUserDefinedPermissions(final TableName tableName,
    ...
    User.runAsLoginUser(new PrivilegedExceptionAction<List<UserPermission>>() {
      ...
      private void getUserPermsFromUserDefinedAccessController(
        final List<UserPermission> userPermissions, AccessControlService.Interface service) {
        ...
        AccessControlProtos.GetUserPermissionsRequest.Builder builderNamespacePerms =
          AccessControlProtos.GetUserPermissionsRequest.newBuilder();
        ...
        AccessControlProtos.GetUserPermissionsRequest requestNamespacePerms =
          builderNamespacePerms.build();
  ...
  /**
   * Authorizes that the current user has all the given permissions for the given table and for the
   * hbase namespace of the table
   * @param tableName Table requested
   * @throws IOException           if obtaining the current user fails
   * @throws AccessDeniedException if user has no authorization
   */
  private void requireAccess(String request, TableName tableName, Action... permissions)
    throws IOException {
    User user = getActiveUser();
    ...
    if (!requiredAccess.isEmpty()) {
      result = AuthResult.deny(request, "Insufficient permissions", user, requiredAccess.get(0),
        tableName, null, null);
    }
    if (!result.isAllowed()) {
      throw new AccessDeniedException("Insufficient permissions " + authString(user.getName(),
        tableName, new HashSet<Permission.Action>(Arrays.asList(permissions))));
    }
  }
   ...
   */
  private boolean hasAccess(List<UserPermission> perms, TableName table, Permission.Action action,
    User user) {
    ...
    if (perms != null) {
      ...
      if (permissionsForUser != null) {
        for (UserPermission permissionForUser : permissionsForUser) {
          if (permissionForUser.getPermission().implies(action)) {
            return true;
          }
        }
      }
      ...
      LOGGER.debug("No permissions found for table=" + table + " or namespace="
  ...
  private void logResult(AuthResult result) {
    if (AUDITLOG.isTraceEnabled()) {
      ...
      AUDITLOG.trace("Access " + (result.isAllowed() ? "allowed" : "denied") + " for user "
        ...
        + (remoteAddr.isPresent() ? remoteAddr.get() : "") + "; request: " + result.getRequest()
  ...
  private static final class Superusers {
    ...
    public static final String SUPERUSER_CONF_KEY =
      org.apache.hadoop.hbase.security.Superusers.SUPERUSER_CONF_KEY; // Not getting a name
    ...
    /**
     * Should be called only once to pre-load list of super users and super groups from
     * Configuration. This operation is idempotent.
    ...
    public static boolean isSuperUser(User user) {
      if (superUsers == null) {
        throw new IllegalStateException(
          "Super users/super groups lists" + " haven't been initialized properly.");
      }
  ...
  public String authString(String user, TableName table, Set<Action> actions) {
    ...
    sb.append("scope=").append(table == null ? "GLOBAL" : table.getNameWithNamespaceInclAsString())
      .append(", ");
